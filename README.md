...
**リポジトリを人に見せる直前に、見せてはいけないものが混ざっていないかを機械で調べる道具です。** GitHub を採用する（作成・push・PR・merge）直前には、検査だけでなく対話ゲートが出ます。

APIキーらしき文字列や自分のPCのパスを、今あるファイルだけでなく**削除済みを含むGitの全履歴から**探します。必須の文書がそろっているか、文書とコードの食い違いがないかも確認します。結果は決まった形式で返るので、人だけでなくAIエージェントやCIからも同じ基準で使えます。GitHub のページ自体が対話 UI になることはありません。`--intent` を付けた CLI が採用時の契約です。
...
GitHub を採用するとき（リポジトリ作成・最初の push・PR・merge）の契約は、この `--intent` 対話です。clone しただけ、GitHub のページを開いただけでは対話は走りません。`--intent` を付けると `repo-preflight.dialogue/v3` の質問リストが機械生成され、答えが返るまで外への操作をしません。GitHub.com がチャット画面になることは保証しません。
